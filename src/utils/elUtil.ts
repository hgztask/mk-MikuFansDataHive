...
//配置默认的验证元素函数
const validationElFun = (config: findElementType, selector: string) => {
...
//私有最后验证的元素
const __privateValidationElFun = (config: findElementType, selector: string) => {
 ...
 */
const findElement = async (selector: string, config: findElementType | any = {}): Promise<any | Element> => {
    const defConfig = {
        doc: document,
        interval: 1000,
        timeout: -1,
 ...
 */
const findElements = async (selector: string, config: findElementType | any = {}):
    Promise<Element[]> => {
 ...
 */
const installStyle = (cssText: string, selector: string = ".mk-def-style") => {
